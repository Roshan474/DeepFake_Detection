...
class ImageDetectorTrainer:
    ...
    def get_new_images_count(self):
        ...
        # Check AI images
        ai_path = os.path.join(self.dataset_path, 'ai')
    ...
    def prepare_dataset_for_training(self, use_only_new=True):
        """Prepare dataset for training without duplicating files"""
        trained_images = self.load_trained_images()
    ...
    def train_with_progress(self, use_only_new=True):
        """Train the model with progress tracking - optimized without file duplication"""
        ...
        try:
            ...
            if use_only_new and model_exists:
                # Check if there are new images to train
                ...
                self.update_progress(5, 'checking', f'Found {new_real_count} new real and {new_ai_count} new AI images')
            
            # Prepare dataset (get file paths without copying)
            training_images, total_images, trained_images = self.prepare_dataset_for_training(use_only_new)
            
            if total_images == 0:
                self.update_progress(100, 'completed', 'No images found for training!', accuracy=0.0)
                return None, 0.0
            
            training_type = "incremental" if (use_only_new and model_exists) else "full"
            self.update_progress(10, 'preparing', f'{training_type.capitalize()} training with {total_images} images...')
