...
class ProfessionalAIDetector:
        ...
        try:
            ...
            for filename in os.listdir(self.ai_text_dir):
                if filename.endswith('.json'):
                    ...
                    with open(filefilepath, 'r', encoding='utf-8') as f:
                        data = json.load(f)
    ...
    def predict(self, text):
        ...
        # NEW STEP 1: Check for spelling errors - if ANY found, likely human written
        ...
        # Step 2: Check for exact match in training data
        exact_match = self._check_exact_match(text)
        if exact_match:
            print("🎯 Exact match found in training data")
            return exact_match
        
        # Step 3: Use ML model if no exact match
