...
class AudioDetectorTrainer:
    ...
    def load_audio_dataset(self, real_path, ai_path, use_only_new=True):
        ...
        if use_only_new:
            ...
            # Load all audio files (first training or forced retrain)
            self.update_progress(10, 'loading', 'Loading all audio files...')
            real_audio = [f for f in os.listdir(real_path) if f.lower().endswith(('.mp3', '.wav', '.m4a', '.flac'))]
            ai_audio = [f for f in os.listdir(ai_path) if f.lower().endswith(('.mp3', '.wav', '.m4a', '.flac'))]
            ...
            for i, audio_file in enumerate(real_audio):
                ...
                progress = 10 + (i / len(real_audio)) * 20
    ...
    def train_with_progress(self, use_only_new=True):
        ...
        try:
            ...
            X, y, audio_count, trained_audio = self.load_audio_dataset(real_path, ai_path, use_only_new=use_only_new)
